...
class ImageFetcher {
    ...
    // In other words, keeping a strong pointer to your instance says:
    // "I'm still interested in its result."
    ...
    private func callHandlerIfNeeded() {
        if fetchFailed, let image = backup,
           let url = image.storeLocallyAsJPEG(named: String(Date()
...
extension String {
    func attributedString(withTextStyle style: UIFont.TextStyle,
        ...
        let font = UIFontMetrics(forTextStyle: .body)
            .scaledFont(for: UIFont.preferredFont(forTextStyle: .body).withSize(size))
