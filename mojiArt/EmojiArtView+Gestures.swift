...
//  mojiArt
//
//  Created by allan galdino on 14/09/21.
//

import UIKit

// MARK: - Gesture Recognition Extension to EmojiArtView

extension EmojiArtView {
    func addEmojiArtGestureRecognizers(to view: UIView) {
        view.isUserInteractionEnabled = true
        view.addGestureRecognizer(UITapGestureRecognizer(target: self,
                                                         action: #selector(selectSubview)))
        view.addGestureRecognizer(UIPanGestureRecognizer(target: self,
                                                         action: #selector(selectAndMoveSubview)))
    }

    var selectedSubview: UIView? {
        get { subviews.filter { $0.layer.borderWidth > 0 }.first }
        set {
    ...
    @objc func selectAndMoveSubview(by recognizer: UIPanGestureRecognizer) {
        ...
        case .changed, .ended:
            if selectedSubview != nil {
                recognizer.view?.center = recognizer.view!.center
                    .offset(by: recognizer.translation(in: self))
    ...
    func enableRecognizers() {
        ...
        if gestureRecognizers == nil || gestureRecognizers!.count == 0 {
            ...
            addGestureRecognizer(UIPinchGestureRecognizer(target: self,
                                                          action: #selector(resizeSelectedLabel)))
        } else {
    ...
    @objc func deselectSubview() { selectedSubview = nil }
